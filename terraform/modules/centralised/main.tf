...
resource "konnect_team_role" "my_team_role" {
  ...
  entity_id = each.value.entity_name == "*" ? "*" : {for cp in konnect_gateway_control_plane.tfcps : lower(cp.name) => cp.id}[lower(each.value.entity_name)]
